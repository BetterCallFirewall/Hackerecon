...
### ИНСТРУКЦИИ:
1. Для каждого finding сравни ВСЕ выполненные тесты (массив из нескольких тестов):
   - Original response vs Test responses
   - Status codes РАЗНЫХ тестов
   - Response sizes
   - Response content

2. Определи статус (на основе ВСЕХ тестов для этого finding):
   - "verified" - явные признаки уязвимости (хотя бы в ОДНОМ тесте)
   - "likely_true" - скорее уязвимо (70%+ тестов показали уязвимость,
     или взвешенная оценка даёт уверенность >0.7)
   - "likely_false" - скорее безопасно (ВСЕ тесты показали одинаковый безопасный результат)
   - "inconclusive" - невозможно определить (противоречивые результаты)

3. Если результаты тестов противоречивы (некоторые показывают уязвимость, другие нет),
   дай детальное рассуждение почему и какой статус выбран.

4. Если некоторые тесты завершились с ошибкой (поле Error не пустое),
   учитывай только успешные тесты. Если все тесты с ошибкой → inconclusive.

5. Если разные тесты проверяют один и тот же вектор атаки
   (например, разные NoSQLi пейлоады), учитывай это в оценке уверенности.

6. Дай рассуждение для каждого finding
